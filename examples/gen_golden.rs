//! Generate golden reference from current decoder output
use std::env;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 2 {
        eprintln!("Usage: cargo run --example gen_golden -- <fixture_id>");
        std::process::exit(1);
    }
    
    let fixture_id = &args[1];
    ...
    std::fs::write(&golden_path, json_string)?;
