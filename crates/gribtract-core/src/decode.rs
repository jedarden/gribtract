...
fn decode_drt3(
    ...
    for g in 0..n_groups {
        ...
        // Use specialized extractor for common widths (branch-free), otherwise
        // fall back to the generic sliding-window extractor.
        let start_bit = bit_offset;
        if !extract_group_specialized(body, start_bit, w, l, gref, &mut packed) {
...
/// Specialized extractor for w=4 (4-bit values, branch-free).
///
/// Each value consumes exactly 4 bits. Two values = 1 byte.
/// This specialization eliminates the while loop by loading 1 byte every 2 iterations.
#[inline]
fn extract_group_w4(
    data: &[u8],
    start_bit: usize,
    ...
    // Load first byte
    let mut buf: u64 = if byte_pos < data.len() { data[byte_pos] as u64 } else { 0 };
    byte_pos += 1;
    let mut buf_bits = 8usize;
    ...
    for _i in 0..count {
        // Refill if needed (every 2 iterations we need a new byte for w=4)
        if buf_bits < 4 {
            if byte_pos < data.len() {
                buf = (buf << 8) | (data[byte_pos] as u64);
                buf_bits += 8;
            }
            byte_pos += 1;
        }
...
/// Specialized extractor for w=8 (8-bit values, branch-free).
///
/// Each value consumes exactly 1 byte.
#[inline]
fn extract_group_w8(
    data: &[u8],
    start_bit: usize,
    ...
    let mut byte_pos = start_bit / 8 + (start_bit % 8 + 7) / 8;
    let skip = start_bit % 8;

    // Handle non-byte-aligned start
    let mut pending: u64 = 0;
    let mut pending_bits = 0usize;

    if skip > 0 {
        if byte_pos > 0 && byte_pos <= data.len() {
            pending = data[byte_pos - 1] as u64;
            pending_bits = 8 - skip;
        }
    }
    ...
    for _i in 0..count {
        if pending_bits >= 8 {
            let v = (pending >> (pending_bits - 8)) & 0xFF;
            pending_bits -= 8;
            out.push(gref + v as i64);
        } else {
            if byte_pos < data.len() {
                let byte = data[byte_pos] as u64;
                if pending_bits > 0 {
                    let v = ((pending << (8 - pending_bits)) | (byte >> pending_bits)) & 0xFF;
                    pending = byte & ((1u64 << pending_bits) - 1);
                    pending_bits = 0;
                    out.push(gref + v as i64);
                } else {
                    out.push(gref + byte as i64);
                }
            } else if pending_bits >= 8 {
                let v = (pending >> (pending_bits - 8)) & 0xFF;
                pending_bits -= 8;
                out.push(gref + v as i64);
            } else {
...
/// Specialized extractor for w=12 (12-bit values, branch-free).
///
/// Each value consumes 12 bits = 1.5 bytes. Two values = 3 bytes.
#[inline]
fn extract_group_w12(
    data: &[u8],
    start_bit: usize,
    count: usize,
    gref: i64,
    ...
    // Load initial bytes (need at least skip + 12 bits = skip + 2 bytes worst case)
    let mut buf: u64 = 0;
    let mut buf_bits = 0usize;

    // Pre-fill
    let init_bytes = (skip + 12 + 7) / 8;
    for _ in 0..init_bytes {
        if byte_pos < data.len() {
            buf = (buf << 8) | (data[byte_pos] as u64);
            buf_bits += 8;
    ...
    for _ in 0..count {
        // For w=12, we need 2 bytes every iteration (worst case)
        while buf_bits < 12 {
            if byte_pos < data.len() {
                buf = (buf << 8) | (data[byte_pos] as u64);
                buf_bits += 8;
...
/// Specialized extractor for w=16 (16-bit values, branch-free).
///
/// Each value consumes exactly 2 bytes.
#[inline]
fn extract_group_w16(
    data: &[u8],
    start_bit: usize,
    count: usize,
    gref: i64,
...
/// Dispatch to specialized extractor based on width.
///
/// Returns true if a specialized extractor was used, false if the generic
/// windowed extractor was used instead.
#[inline]
fn extract_group_specialized(
    ...
    match w {
        4 => {
            extract_group_w4(data, start_bit, count, gref, out);
            true
        }
        8 => {
            extract_group_w8(data, start_bit, count, gref, out);
            true
        }
        12 => {
            extract_group_w12(data, start_bit, count, gref, out);
            true
        }
        16 => {
            extract_group_w16(data, start_bit, count, gref, out);
            true
        }
        _ => false, // Use generic path
    }
}
