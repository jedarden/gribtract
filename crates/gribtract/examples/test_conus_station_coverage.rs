...
fn main() {
    ...
    let synthetic_bytes = fs::read(synthetic_path)
        .expect("Failed to read synthetic CONUS DRT=0 fixture");

    let synthetic_fields = gribtract::decode(&synthetic_bytes)
        .expect("Failed to decode synthetic fixture");

    println!("Decoded {} field(s) from synthetic fixture\n", synthetic_fields.len());

    for (i, field) in synthetic_fields.iter().enumerate() {
        println!("Field {}:", i);
        println!("  Grid: {} points ({} x {})", field.grid.num_data_points, field.grid.nx, field.grid.ny);
            ...
            match field.grid.nearest_index(lat, lon) {
                Some(idx) => {
                    ...
                    if field.values.get_at(idx).is_some() {
                        ...
                        out_of_range_stations.push((name, lat, lon, "No value at index".to_string()));
                        println!("    ❌ {}: ({:.4}, {:.4}) -> No value at index {}", name, lat, lon, idx);
                    }
                }
                None => {
                    out_of_range_stations.push((name, lat, lon, "No grid index".to_string()));
                    println!("    ❌ {}: ({:.4}, {:.4}) -> Not in grid bounds", name, lat, lon);
                }
            }
        }

        println!("\n  Coverage: {}/{} stations in range ({:.1}%)",
            in_range, STATIONS.len(),
            (in_range as f64 / STATIONS.len() as f64) * 100.0);

        if out_of_range_stations.is_empty() {
            println!("  ✅ All {} US stations covered by synthetic CONUS DRT=0!", STATIONS.len());
        } else {
            println!("  ⚠️  {} station(s) NOT covered:", out_of_range_stations.len());
    ...
    let old_bytes = fs::read(old_global_path)
        .expect("Failed to read old global fixture");

    let old_fields = gribtract::decode(&old_bytes)
        .expect("Failed to decode old global fixture");

    println!("Old global fixture grid: {}°N-{}°N, {}°E-{}°E ({}x{} = {} points)",
        old_fields[0].grid.lat_first, old_fields[0].grid.lat_last,
        old_fields[0].grid.lon_first, old_fields[0].grid.lon_last,
        old_fields[0].grid.nx, old_fields[0].grid.ny,
        old_fields[0].grid.num_data_points);

    let mut old_in_range = 0usize;
    for &(name, lat, lon) in STATIONS {
        match old_fields[0].grid.nearest_index(lat, lon) {
            Some(_) => { old_in_range += 1; println!("  ✅ {}: ({:.4}, {:.4}) IN RANGE", name, lat, lon); },
            None => println!("  ❌ {}: ({:.4}, {:.4}) OUT OF RANGE", name, lat, lon),
        }
    }
    println!("Old global fixture coverage: {}/{} stations ({}%)\n", old_in_range, STATIONS.len(),
    ...
    if fs::metadata(large_path).is_ok() {
        ...
        let large_bytes = fs::read(large_path)
            .expect("Failed to read large GFS CONUS fixture");

        println!("File size: {:.1} MB\n", large_bytes.len() as f64 / 1_048_576.0);

        let large_fields = gribtract::decode(&large_bytes)
            .expect("Failed to decode large fixture");

        println!("Decoded {} field(s) from large fixture\n", large_fields.len());
        ...
        for (i, field) in large_fields.iter().take(3).enumerate() {
            ...
            println!("  Coverage: {}/{} stations in range ({:.1}%)",
                in_range, STATIONS.len(),
                (in_range as f64 / STATIONS.len() as f64) * 100.0);

            if out_of_range == 0 {
                println!("  ✅ All {} US stations covered by GFS CONUS DRT=0!", STATIONS.len());
            } else {
    ...
    println!("\n=== Summary ===");
    println!("Station roster contains {} US weather stations", STATIONS.len());
