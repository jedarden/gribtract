...
fn verify_lambert_gdt30_metadata_population() {
    ...
    assert_eq!(
        ...
        "Total points should be 262,792"
    );
    ...
    assert_eq!(params.dx_m, 12191.0, "Dx should be 12191.0 meters");

    // Dy (grid spacing in y-direction) - 12191 meters
    assert_eq!(params.dy_m, 12191.0, "Dy should be 12191.0 meters");
    ...
    assert!(
        (first_grid.lat_first - 12.19).abs() < 0.001,
        "First latitude should be ~12.19° N, got {}",
        first_grid.lat_first
    );
    assert!(
        (first_grid.lon_first - 226.541).abs() < 0.001,
        "First longitude should be ~226.541° E, got {}",
        first_grid.lon_first
    );
    ...
    for (i, field) in fields.iter().enumerate().skip(1) {
        assert_eq!(
            field.grid.template, first_grid.template,
            "Field {}: GDT template mismatch",
            i
        );
        assert_eq!(field.grid.nx, first_grid.nx, "Field {}: Nx mismatch", i);
        assert_eq!(field.grid.ny, first_grid.ny, "Field {}: Ny mismatch", i);
        assert_eq!(
            field.grid.num_data_points, first_grid.num_data_points,
        ...
        assert!(
            (field_params.lad - params.lad).abs() < 1e-9,
            "Field {}: LaD mismatch",
            i
        );
        assert!(
            (field_params.lov - params.lov).abs() < 1e-9,
            "Field {}: LoV mismatch",
            i
        );
        assert!(
            (field_params.dx_m - params.dx_m).abs() < 1e-6,
            "Field {}: Dx mismatch",
            i
        );
        assert!(
            (field_params.dy_m - params.dy_m).abs() < 1e-6,
            "Field {}: Dy mismatch",
            i
        );
    }
    ...
    println!(
        "✅ All {} fields have consistent Lambert 3.30 grid metadata",
        fields.len()
    );
}
