#!/usr/bin/env python3
"""
Generate a minimal valid GRIB2 fixture using DRT=40 (template 5.40, JPEG 2000
data compression) for gribtract testing.

Message: 2m temperature analysis, 3x2 lat/lon grid (0-20°N, 0-20°E, 10° spacing),
JPEG2000 J2K-codestream section 7, no bitmap.

Packing: R=100.0, E=0, D=0, N=8 (8-bit unsigned, lossless J2K).
...
def make_j2k_codestream(width, height, pixels):
    """
    Generate a JPEG 2000 codestream (J2K) using PIL.

    PIL's JPEG2000 encoder wraps OpenJPEG and defaults to lossless (reversible
    5/3 DWT). The output is a JP2 container; we extract the embedded J2K
    codestream by scanning for the SOC marker (0xFF 0x4F).
    """
    arr = np.array(pixels, dtype=np.uint8).reshape(height, width)
    img = PILImage.fromarray(arr, mode='L')
    buf = io.BytesIO()
    # irreversible=False → lossless reversible encoding (5/3 DWT)
    img.save(buf, format='JPEG2000', irreversible=False)
    jp2_data = buf.getvalue()

    # Extract J2K codestream: starts at SOC marker (FF 4F), ends at EOC (FF D9).
    soc = b'\xff\x4f'
    soc_pos = jp2_data.find(soc)
    if soc_pos < 0:
        raise RuntimeError("SOC marker not found in JP2 output")
    j2k = jp2_data[soc_pos:]

    # Sanity-check: J2K codestream magic is FF 4F FF 51 (SOC then SIZ marker).
    assert j2k[:4] == b'\xff\x4f\xff\x51', f"Bad J2K magic: {j2k[:4].hex()}"
    assert j2k[-2:] == b'\xff\xd9', f"Bad EOC: {j2k[-2:].hex()}"
    return j2k
...
# ── Section 5: Data Representation (DRT 5.40 — JPEG2000 compression) ──────────
# Template 5.40 uses the common packing header (same as DRT=0):
# R=100.0, E=0, D=0, N=8, type=0
s5_body = (
    u32(N_POINTS)   +   # number of packed values
    u16(40)         +   # DRT template: 5.40 (JPEG2000 compression)
    f32(100.0)      +   # R: reference value
    grib_i16(0)     +   # E: binary scale factor = 0
    grib_i16(0)     +   # D: decimal scale factor = 0
    u8(8)           +   # N: bits per value (8-bit)
...
# ── Section 7: JPEG2000 J2K codestream ────────────────────────────────────────
# Pixel values X = [0, 1, 2, 3, 4, 5] for the 3x2 grid.
# Row 0 (lat=10°N): [0, 1, 2]
# Row 1 (lat= 0°N): [3, 4, 5]
pixels = list(range(N_POINTS))
j2k_bytes = make_j2k_codestream(NI, NJ, pixels)
print(f"J2K codestream size: {len(j2k_bytes)} bytes")
print(f"J2K magic: {j2k_bytes[:4].hex()}")
...
out_path = os.path.join(repo_root, 'tests', 'corpus', 'small', 'drt40_j2k_3x2.grib2')
...
golden = {
    "fixture_id": "drt40_j2k_3x2",
    "_provenance": (
        "Synthetic 3x2 DRT=40 (JPEG 2000 data compression). "
        "Generated by scripts/gen_grib2_drt40.py. "
        "Packing: R=100.0, E=0, D=0, N=8 (8-bit lossless J2K). "
...
golden_path = os.path.join(repo_root, 'tests', 'corpus', 'golden', 'drt40_j2k_3x2.json')
...
print(json.dumps({
    ...
    "provenance": {
        "source": "synthetic",
        "description": (
            f"Minimal GRIB2 DRT=40 fixture: 3x2 lat/lon, JPEG2000 J2K codestream, "
            f"R=100.0 E=0 D=0 N=8 (lossless). Pixels X=[0..5] -> Y=[100..105]. "
            f"Section 5 template 5.40 (JPEG 2000 data compression)."
        ),
        "capture_date": "2026-06-21",
        "generated_by": "scripts/gen_grib2_drt40.py (gribtract project)"
    }
}, indent=2))
