...
Packing: R=100.0, E=0, D=0, N=32 (bits per value for IEEE 32-bit floats).
...
# Common header (octets 12-21) + reserved (octets 22-23, must be 0)
s5_body = (
    u32(N_POINTS)   +   # number of packed values
    u16(4)          +   # DRT template: 5.4 IEEE 32-bit floats
    f32(100.0)      +   # R: reference value (not used for IEEE floats)
    grib_i16(0)     +   # E: binary scale factor = 0
    grib_i16(0)     +   # D: decimal scale factor = 0
    u8(32)          +   # N: bits per value = 32 (IEEE 32-bit floats)
    u8(0)           +   # type of original field: 0 = floating point
    u8(0)           +   # octet 22: reserved (must be 0)
    u8(0)               # octet 23: reserved (must be 0)
)
...
print(json.dumps({
    ...
    "provenance": {
        "source": "synthetic",
        "description": f"Minimal GRIB2 DRT=4 fixture: 3x3 lat/lon, IEEE 754 32-bit floats. R=100.0 E=0 D=0 N=32. Values: raw IEEE 32-bit floats [100.0..108.0]. Section 5 template 5.4 (IEEE 754 32-bit floats).",
